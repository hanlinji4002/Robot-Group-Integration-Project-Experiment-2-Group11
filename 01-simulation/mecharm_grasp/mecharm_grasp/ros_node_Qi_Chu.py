#!/usr/bin/env python3
"""抓取流程 Qi_Chu（独立副本）。与流程 1 完全不共享代码，改这里不影响 ros_node2.py。
...
class GraspTask(Node):
    def __init__(self):
        super().__init__('grasp_task_Qi_Chu')
        for name, default in [
            ('base_pose_xyz', [0.0, 0.0, 0.75]), ('point_a', [0.10, 0.10, 0.7635]),
            ('point_b', [0.10, -0.10, 0.7635]), ('safe_height', 0.05),
            ('tool_tip_offset', 0.063), ('gripper_open', 0.0), ('gripper_close', -0.44),
            ('move_duration', 2.5), ('grip_duration', 1.2), ('cycles', 5),
            ('log_dir', '~/grasp_logs_Qi_Chu'), ('world_name', 'grasp_world_Qi_Chu'),
            ('sim_attach', False), ('sim_check', True),
        ]:
