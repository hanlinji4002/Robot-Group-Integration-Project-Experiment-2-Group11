...
│   │   ├── grasp_chendezhong_grasp3.yaml  抓取参数（chendezhong_grasp3）：反向 A3/B3 点与独立日志目录
...
│       └── sim_chendezhong_grasp3.launch.py  一键启动 chendezhong_grasp3 流程（复用流程 1 控制算法）
...
| 节点名 | grasp_task | grasp_task2 | grasp_task3 | grasp_task_chendezhong_grasp3 |
| 世界名 | grasp_world | grasp_world2 | grasp_world3 | grasp_world_chendezhong_grasp3 |
| 日志目录 | ~/mecharm_ws/grasp_logs | ~/grasp_logs2 | ~/grasp_logs3 | ~/grasp_logs_chendezhong_grasp3 |
| 验证状态 | Jetson 5/5 | Jetson 5/5 | Jetson 5/5 | Jetson 5/5 |

四套均已在 Jetson 上实测 5 次抓取全部成功。流程 3 的验收日志已提交；chendezhong_grasp3（陈德中）
...
运行 chendezhong_grasp3 反向对角搬运流程：

```
ros2 launch mecharm_grasp sim_chendezhong_grasp3.launch.py gui:=false
```

chendezhong_grasp3 流程的成绩单：

```
cat ~/grasp_logs_chendezhong_grasp3/summary.txt
```
...
流程 3 是独立的斜向搬运场景，沿用流程 2 的 IK、轨迹动作、夹爪、状态机和日志逻辑。
A3=[0.095, 0.095, 0.7635]，B3=[0.100, -0.090, 0.7635]，safe_height=0.05 m；世界中物体初始 pose 与 A3 一致。
...
| 跑 chendezhong_grasp3，不看画面 | `ros2 launch mecharm_grasp sim_chendezhong_grasp3.launch.py gui:=false` |
| 看 chendezhong_grasp3 成绩单 | `cat ~/grasp_logs_chendezhong_grasp3/summary.txt` |
