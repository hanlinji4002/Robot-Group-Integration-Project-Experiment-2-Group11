...
仿真部分提供三套互不覆盖日志的抓取流程。流程 3 复用流程 1 已验证的控制算法，
通过独立参数、世界和节点重命名改变任务场景。
...
仿真代码按职责拆成四个模块。三套场景数据与日志相互独立；流程 3 共享流程 1
的控制算法，算法修复会同时作用于这两个流程。

**模型构建（`model/`）** 描述"机械臂长什么样、世界里有什么"。`arm_model.xacro` 是 mechArm 270 的模型，取自厂商公开的官方描述包，我们在其基础上补全了惯量、关节限位，并把夹爪的联动关节接进 ros2_control。`theWorld*.sdf` 是三套 Gazebo 仿真世界，均包含桌面、25 mm 的目标方块以及物理和里程计插件。
